...
class PandasNgram(object):
    ...
    def removeNoneTypes(self, lst):
        yield ([i for i in lst if type(i) is not types.NoneType])
        #return [i for i in lst if type(i) is not types.NoneType]
        #if lst:
            #yield ([i for i in lst if not isinstance(i, type(None))])
            #yield ([i for i in lst if not isinstance(i, types.NoneType)])
    ...
    def cleanText(self, prefix=None):
        ...
        try:
            ...
            print("UnicodeDecodeError exception thrown")
    ...
    def tokenize_sentences(self, prefix, **kwargs):
        ...
        #sentences = self.data[prefix].map(lambda s: s.sentences.pop().strip())

        sentences = self.data[prefix].map(lambda s: s.sentences)

        #rlen = len(self.data[prefix])
        #sentences = self.data[prefix].copy()
        #print(type(sentences))
        #for line in range(0,rlen):
        #    tmp = self.data[prefix][line].sentences.pop().strip()
        #    sentences[line] = tmp
    ...
    def tokenize_words(self, prefix, normalize = 'stem', **kwargs):
        ...
        words = self.data[prefix].map(lambda l: map(lambda w: w.strip().tokens if len(w)>1 else None, l))
...
ngram_container = list()
for gram in resumes.ngram_collection['experience_tokens']:
    ngram_container.extend(gram)
for gram in resumes.ngram_collection['experience_bigrams']:
    ngram_container.extend(gram)
for gram in resumes.ngram_collection['experience_trigrams']:
    ngram_container.extend(gram)
for gram in resumes.ngram_collection['experience_fourgrams']:
    ngram_container.extend(gram)
print(len(ngram_container))
print(len(set(ngram_container)))
